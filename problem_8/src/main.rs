...
fn get_number() -> Vec<i64> {
    let number: Vec<i64> =
        vec![ 7, 3, 1, 6, 7, 1, 7, 6, 5, 3, 1, 3, 3, 0, 6, 2, 4, 9, 1, 9
...
fn product(index: i64, limit: i64) -> i64 {
    ...
    let mut prod: i64 = 1;
    for i in index..index+limit {
        prod *= number[i as usize];
    }
    return prod;
}

fn max_prod(limit: i64) -> i64 {
    let mut index: i64 = 0;
    let mut prod: i64;
    let mut new_max: i64 = product(index, limit);
    while index < (1000-limit) {
        prod = product(index, limit);
        if prod >= new_max {
            new_max = prod;
        }
        index += 1;
